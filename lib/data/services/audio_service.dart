...
/// Audio encryption service with AES-256-CBC encryption
class AudioService {
  ...
  // Encrypt audio file using AES-256-CBC
  static Future<File> encodeFile(
    ...
    try {
      ...
      throw Exception('خطا در رمزگذاری صدا: $e');
    }
  }

  // Decrypt audio file using AES-256-CBC
  static Future<File> decodeFile(
    ...
    try {
      ...
      if (lastDot > 0) {
        extension = originalName.substring(lastDot); // e.g., .m4a
        baseName = originalName.substring(0, lastDot); // e.g., audio_encrypted or audio_1234567890_12345
      } else {
      ...
      randomName += "_${DateTime.now().millisecondsSinceEpoch}";
      randomName += "_${random.nextInt(100000)}";
      randomName += extension.isNotEmpty ? extension : '.m4a'; // Default to .m4a if no extension
      ...
      throw Exception('خطا در رمزگشایی صدا: $e');
    }
  }
}

// Export functions for backward compatibility
Future<File> encodeAudioFile(File inputFile, String key, {String? fileName}) {
  return AudioService.encodeFile(inputFile, key, fileName: fileName);
}

Future<File> decodeAudioFile(File encodedFile, String key, {String? fileName}) {
  return AudioService.decodeFile(encodedFile, key, fileName: fileName);
}
